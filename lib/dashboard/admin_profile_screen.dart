  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          "Control Center",
          style: TextStyle(fontWeight: FontWeight.w900, color: Colors.black, fontSize: 24, letterSpacing: -1),
        ),
        actions: [
          Container(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(text: "REVIEWS", icon: Icon(Icons.chat_bubble_outline_rounded, size: 20)),
            Tab(text: "PROFILE", icon: Icon(Icons.account_circle_outlined, size: 20)),
          ],
        ),
      ),
      body: TabBarView(
        ...
        builder: (context, child) {
          return _tabController.index == 0
              ? FloatingActionButton.extended(
                  ...
                  label: const Text("NEW ENTRY", style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1)),
              ...
              child: ListTile(
                ...
                leading: ClipRRect(
                  ...
                  child: Image.network(data['image'] ?? '', width: 60, height: 60, fit: BoxFit.cover, 
                    errorBuilder: (c, e, s) => Container(color: Colors.grey[50], width: 60, child: const Icon(Icons.image_not_supported_outlined))),
                ),
                title: Text(data['name'] ?? 'Untitled', style: const TextStyle(fontWeight: FontWeight.w800)),
                subtitle: Text("${data['cityName']} • ${data['category']}", style: TextStyle(color: Colors.grey[500], fontSize: 12)),
                trailing: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    IconButton(icon: const Icon(Icons.edit_note_rounded, color: Colors.blueAccent), 
                      onPressed: () => showModalBottomSheet(context: context, isScrollControlled: true, builder: (_) => AddPlaceFormSheet(doc: doc))),
                    IconButton(icon: const Icon(Icons.delete_outline_rounded, color: Colors.redAccent), 
  ...
  void _confirmDelete(BuildContext context, DocumentSnapshot doc) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("This information will be permanently removed from the database."),
        actions: [
...
class _AddPlaceFormSheetState extends State<AddPlaceFormSheet> {
  ...
  Future<void> _submitData() async {
    if (_nameController.text.isEmpty || _cityNameController.text.isEmpty) return;
    setState(() => _isLoading = true);
    try {
      String finalImageUrl = _existingImageUrl ?? '';
      if (_selectedImage != null) {
        final uri = Uri.parse('https://api.cloudinary.com/v1_1/dbqrxk5ya/image/upload');
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(widget.doc == null ? "ADD NEW PLACE" : "EDIT PLACE", style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: Colors.grey[50], borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.grey.shade100, width: 2)),
                child: _selectedImage != null 
                  ? ClipRRect(borderRadius: BorderRadius.circular(18), child: Image.file(_selectedImage!, fit: BoxFit.cover))
                  : (_existingImageUrl != null 
                      ? ClipRRect(borderRadius: BorderRadius.circular(18), child: Image.network(_existingImageUrl!, fit: BoxFit.cover))
                      : Icon(Icons.add_a_photo_outlined, color: Colors.grey[300], size: 40)),
              ),
            ),
            const SizedBox(height: 25),
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: Colors.black, minimumSize: const Size(double.infinity, 60), elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18))),
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide(color: Colors.grey.shade100, width: 1.5)),
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Container(
              ...
              child: ListTile(
                leading: CircleAvatar(backgroundColor: Colors.grey[100], child: const Icon(Icons.person_outline, color: Colors.black)),
                title: Text(data['name'] ?? 'User', style: const TextStyle(fontWeight: FontWeight.bold)),
                subtitle: Text(data['email'] ?? ''),
                trailing: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(color: Colors.blueAccent.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                  child: Text(data['role']?.toUpperCase() ?? 'USER', style: const TextStyle(fontSize: 10, color: Colors.blueAccent, fontWeight: FontWeight.bold)),
                ),
              ),
            );
          },
...
class ManageReviewsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
        if (snapshot.data!.docs.isEmpty) return const Center(child: Text("No reviews yet."));
        return ListView.builder(
          padding: const EdgeInsets.all(20),
          itemCount: snapshot.data!.docs.length,
          itemBuilder: (context, index) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade100), borderRadius: BorderRadius.circular(15)),
              child: ListTile(
                title: Text("\"${data['comment']}\"", style: const TextStyle(fontStyle: FontStyle.italic, fontSize: 13)),
                subtitle: Text("${data['stars']} ⭐", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange)),
                trailing: IconButton(icon: const Icon(Icons.delete_sweep_outlined, color: Colors.redAccent), onPressed: () => doc.reference.delete()),
              ),
            );
          },
        );
...
class _AdminProfileTabState extends State<AdminProfileTab> {
  ...
  Future<void> _updateProfilePic() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(_uid).update({'profilePic': newImageUrl});
    } catch (e) { debugPrint(e.toString()); }
              ...
              Stack(
                ...
                children: [
                  Container(
                    ...
                    child: CircleAvatar(radius: 60, backgroundColor: Colors.grey[50], backgroundImage: userData['profilePic'] != null ? NetworkImage(userData['profilePic']) : null,
                      child: userData['profilePic'] == null ? const Icon(Icons.person, size: 50, color: Colors.black) : null),
                  ),
                  GestureDetector(
                    onTap: _isUploading ? null : _updateProfilePic,
                    child: CircleAvatar(radius: 18, backgroundColor: Colors.black, child: _isUploading ? const SizedBox(width: 15, height: 15, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.camera_alt, size: 16, color: Colors.white)),
                  ),
                ],
              ),
              const SizedBox(height: 30),
              _profileCard("Name", userData['name'] ?? 'Admin', Icons.badge_outlined),
              _profileCard("Email", userData['email'] ?? '', Icons.alternate_email),
              _profileCard("Status", "System Administrator", Icons.verified_user_outlined),
            ],
          ),
        );
      },
  ...
  Widget _profileCard(String title, String val, IconData icon) {
    return Container(
      ...
      decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade100), borderRadius: BorderRadius.circular(18)),
      child: Row(
        children: [
